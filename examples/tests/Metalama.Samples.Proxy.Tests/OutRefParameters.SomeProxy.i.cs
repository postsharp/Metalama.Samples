...
namespace Metalama.Samples.Proxy.Tests
{
  public class SomeProxy : ISomeInterface
  {
    ...
    static SomeProxy()
    {
      _metadata1 = new InterceptionMetadata(typeof(ISomeInterface).GetMethod("VoidMethod", BindingFlags.Public | BindingFlags.Instance, null, new[] { typeof(int).MakeByRefType(), typeof(string).MakeByRefType(), typeof(DateTime).MakeByRefType(), typeof(TimeSpan).MakeByRefType() }, null) ?? throw new MissingMethodException("The method 'ISomeInterface.VoidMethod(out int, ref string, in DateTime, ref readonly TimeSpan)' could not be found using reflection."), false);
      _metadata2 = new InterceptionMetadata(typeof(ISomeInterface).GetMethod("NonVoidMethod", BindingFlags.Public | BindingFlags.Instance, null, new[] { typeof(int).MakeByRefType(), typeof(string).MakeByRefType(), typeof(DateTime).MakeByRefType(), typeof(TimeSpan).MakeByRefType() }, null) ?? throw new MissingMethodException("The method 'ISomeInterface.NonVoidMethod(out int, ref string, in DateTime, ref readonly TimeSpan)' could not be found using reflection."), false);
    }
