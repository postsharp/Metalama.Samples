...
public readonly struct TransactionalObjectId : IEquatable<TransactionalObjectId>
{
    ...
    public override bool Equals( object? obj ) =>
        obj is TransactionalObjectId other && this.Equals( other );
...
public readonly struct TransactionalObjectId<T> : IEquatable<TransactionalObjectId<T>>,
    IEquatable<TransactionalObjectId>
    where T : ITransactionalObject
{
    ...
    public bool Equals( TransactionalObjectId<T> other ) =>
        this._underlying.Equals( other._underlying );

    public bool Equals( TransactionalObjectId other ) => this._underlying.Equals( other );

    public override bool Equals( object? obj ) =>
        obj switch
        {
            TransactionalObjectId<T> other => this.Equals( other ),
            TransactionalObjectId other => this.Equals( other ),
            _ => false
        };

    public override int GetHashCode() => this._underlying.GetHashCode();
